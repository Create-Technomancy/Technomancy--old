...
//        BlockEntity te = world.getBlockEntity(pos);
        ...
        for (Direction d : Iterate.directions) {
            AetherTransportBehaviour.AttachmentTypes attachment = getRenderedRimAttachment(world, pos, state, d);
            if (attachment != AetherTransportBehaviour.AttachmentTypes.NONE) {
                connectedSides++;
                connectedSide = d;
            }
        ...
        if (connectedSides == 0) {
            data.putAttachment(Direction.NORTH, AetherTransportBehaviour.AttachmentTypes.RIM);
            data.putAttachment(Direction.SOUTH, AetherTransportBehaviour.AttachmentTypes.RIM);
        } else if (connectedSides == 1) {
            data.putAttachment(connectedSide.getOpposite(), AetherTransportBehaviour.AttachmentTypes.RIM);
        }
...
//                data.putAttachment(d, ((CableBlockEntity)te).getRenderedRimAttachment(world, pos, state, d));
    ...
    private AetherTransportBehaviour.AttachmentTypes getRenderedRimAttachment(BlockAndTintGetter world, BlockPos pos, BlockState state, Direction d) {
        Block otherBlock = world.getBlockState(pos.relative(d)).getBlock();
        if (otherBlock instanceof CableBlock)
            return AetherTransportBehaviour.AttachmentTypes.NORMAL;
        else if (otherBlock instanceof AetherNetworkBlock)
            return AetherTransportBehaviour.AttachmentTypes.RIM;
        else
            return AetherTransportBehaviour.AttachmentTypes.NONE;
    }
